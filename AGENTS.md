...
demo/                    # Grounding demo, its own module (github.com/vrinek/Dialog/demo)
  ...
  internal/chainfile/    # reads and writes the committed chain directory
  internal/replay/       # the node's loading path: validate → L2 → L3
  internal/render/       # an entity read back as the English sentence it spells
...
`demo/` is a second Go module (`github.com/vrinek/Dialog/demo`) requiring the
library through a `replace` onto `../go`, so that the MCP SDK it needs stays out
of the library's dependency surface. Run all five from `demo/`, and all five
...
`demo/chains/` holds generated bytes under the same rule as `vectors/`: never
hand-edited, regenerated with `go run ./cmd/genchains`, and byte-identical every
time. A change there moves every digest the demo's README and tests quote.
CI runs the same five in [`demo.yml`](.github/workflows/demo.yml), with `-race`.

Changes to `go/` can break this module and are not caught by `go/`'s own suite;
run the demo's tests too when the library's L2 or L3 behaviour moves. See
