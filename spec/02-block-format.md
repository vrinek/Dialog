...
| `refs` | `[* bstr .size 32]` | Zero or more SHA-256 digests of CID-providing blocks (blocks whose operations define entities needed by this block). MAY be empty. Entries MUST be pairwise distinct, and MUST NOT name a block of the author's own chain. A public block's entries MUST NOT name a private block. The order of the entries carries no meaning. See [The refs list](#the-refs-list) and [Validation](#validation). |
...
- Only one chain can succeed a rotation. If a node holds more than one genesis block referencing the same rotation block, the succession is ambiguous: the node MUST surface the conflict and MUST NOT pick a successor on its own (see [05-processing-model.md](05-processing-model.md), "Chain succession (key rotation)"). Such genesis blocks are a fork in the strict sense of rule 9 as well — they are distinct blocks signed by the successor key, all claiming the genesis position of its chain — but this position is held to a stricter rule than rule 9's. Accept-first-seen, which rule 9 leaves open for an ordinary fork, is not available here: a node MUST NOT treat any one claimant as *the* successor — for auto-subscription, for block order across the junction, or for any other purpose — while the ambiguity stands. It MAY store and serve every claimant, which is not a choice between them; resolving the ambiguity is the application's business and not the node's. A silent choice between two chains that both claim a rotated key would be exactly the unproven chain of custody verifiable succession exists to refuse.
...
A block whose ancestry is not available locally is neither valid nor invalid. It is **stored but unvalidated** (see [05-processing-model.md](05-processing-model.md), "Block reception"): the node may hold its bytes, but it MUST NOT treat the block as valid and MUST NOT let its operations reach L2 until the missing ancestors arrive and the block validates. The same holds for rule 4: a block whose reference resolution needs a block the node cannot read — one it does not hold, or one it holds without the key to it — and which leaves a digest unresolved for that reason, is stored but unvalidated in exactly this sense and MUST NOT be recorded as invalid (see rule 4 below). Validity in this sense is relative to what a node holds — two nodes may disagree about a block until both hold everything validating it requires — but the disagreement is between "valid" and "undecided", never between "valid" and "invalid": the absence of a block is not evidence about the validity of the block that names it.

**A verdict moves in one direction.** A block a node has accepted as valid stays valid. Learning more later MAY be surfaced to the application and MUST NOT turn an accepted block into an invalid one: what a validation did not check does not become a defect when the node's store grows. Rules 6 and 10 are reported as *unchecked* for a `refs` entry a validation did not resolve, and such an entry is permanently outside that block's validity (see rules 6 and 10 below). The undecided verdict is the one that is expected to move: a stored but unvalidated block becomes valid — or invalid — once what it was waiting for arrives.
...
4. **Operation validity.** Every operation in `ops` MUST reference only entity digests that are **reachable** — defined in:
   ...
   The recursion of the third branch is bounded by the **scan limit** defined in [05-processing-model.md](05-processing-model.md), "Scan limit": a block whose digests do not all resolve within it is invalid for unresolvable references. The limit counts distinct foreign blocks scanned, is user-configurable, and defaults to 256 — the one value in this specification an implementation chooses that decides validity, which is why the counting unit and the default are both written down.

   **This rule's verdict is three-valued.** Resolution either finds a digest, proves it absent, or cannot say, and only the middle outcome is a rejection (see [05-processing-model.md](05-processing-model.md), "Resolution procedure"):

   - **Resolvable.** Every digest an operation names resolves through one of the three branches. The rule passes.
   - **Definitively unresolvable — invalid.** Resolution completed against the blocks the node holds and the digest is provably absent from the reachable set: every `refs` entry was held and scanned, every block reached transitively through them was read, the author's own chain was walked, and no operation creates the digest. The scan limit being reached is the other definitive rejection: it is a bound the node chose, applied to blocks it holds, and the same bound in every default-configured node.
   - **Not determinable — stored but unvalidated.** A block resolution needs cannot be read — a `refs` entry, a block reached transitively through one, or an ancestor of the author's own chain that is either not held and unobtainable, or held as ciphertext this node has no decryption key for (see [05-processing-model.md](05-processing-model.md), "Undecryptable reference handling") — and a digest is still unresolved when resolution runs out of blocks it can read. The block is neither valid nor invalid: it is **stored but unvalidated**, exactly as a block whose `prev` has not arrived is. It MUST NOT reach L2 and MUST NOT be treated as another block's predecessor under rule 3; a node that keeps it SHOULD revalidate it when the missing block, or the missing key, arrives — whatever verdict the arriving block itself receives (see [05-processing-model.md](05-processing-model.md), "Block reception", "Revalidation on arrival") — and MAY discard it instead. A node MUST NOT record such a block as invalid: neither the absence of a block nor the absence of a key is evidence about validity, and a source able to turn a withheld block into a rejection could invalidate blocks it does not like (see [05-processing-model.md](05-processing-model.md), "Block reception", "Absence is not evidence").

   **The three branches name blocks, not verdicts.** A definition MAY be read from any block the node holds and can read that passes the block's self-contained checks — canonical bytes, structure, and signature, the checks a store performs on admission — whatever that block's own chain standing under rules 3 and 9, and whether or not the node has decided about it at all. A definition is self-certifying: an operation defines a digest only if the entity's canonical bytes hash to it, which the reader establishes for itself, and the source block's standing cannot change what those bytes hash to. This is the one place the rule departs from rule 3, which does require its target to have been accepted as valid. See [05-processing-model.md](05-processing-model.md), "Resolution procedure", for the rule, its reason and its boundaries — in particular that L2 accumulation still admits only valid blocks, and that rules 6 and 10 below still apply to the referenced block as written.
...
6. **Public/private reference rules.** A public block's `refs` MUST NOT name a private block. A private block's `refs` MAY name a block of any type. The rule names the one target it excludes rather than enumerating the permitted ones: what a public block may not depend on is content that a node without a decryption key cannot read, and that is a private block and nothing else. A public block MAY therefore reference public blocks and rotation blocks — a rotation block is in the clear for every node — which is what the genesis block of a successor chain does when it names the rotation block it succeeds (see [rotate_key](#rotate_key), "Verifiable succession"). The rule is evaluated on each referenced block as it is resolved: a node MUST reject a public block once it holds a private block that the public block's `refs` name, and reports the rule as unchecked for an entry whose block it does not hold. Resolution is demand-driven (see [05-processing-model.md](05-processing-model.md)), so a node is not obliged to fetch a referenced block for the sole purpose of reading its type.

   *Unchecked* is informational and nothing else: it names the entries a verdict does not cover, so that an application can ask for them if it cares. It is not a defect, not a reservation on the block's validity, and not a state the block carries — a block whose other rules passed is valid, reaches L2 and may be served. The rule binds only for the entries a validation of that block resolved. An entry no validation resolved is permanently outside that block's validity: a node that later discovers — while validating another block, say — that such an entry names a private block MUST NOT re-open the verdict of a block it has already accepted, and nothing is asked of the entities that block contributed to L2. The asymmetry with rule 4's third outcome is deliberate. An unresolved digest means the node cannot show the block **sound**, and a block it cannot show sound must not reach L2; an unchecked entry means only that it has not found the block **unsound**, and treating "no violation found" as a reason to withhold a block would keep every block with an unfetched `refs` entry out of L2 forever.
...
9. **Fork detection.** If a node receives a block whose `prev` value matches the `prev` of another block already stored from the same `pub` key, the node MUST detect this as a chain fork. Fork handling strategy (reject, flag, accept-first-seen) is implementation-scoped. One case is held to a stricter rule: more than one genesis block claiming the same rotation block is a fork in this sense, and accept-first-seen is not available for it (see [rotate_key](#rotate_key), "Verifiable succession").
10. **Reference hygiene.** The `refs` list MUST NOT repeat a digest, and MUST NOT name a block of the author's own chain. See [The refs list](#the-refs-list). The duplicate half is structural and is checked when the block is decoded; the own-chain half is checked on each referenced block as it is resolved, like rule 6, and carries rule 6's *unchecked* status and its binding scope — an entry no validation of the block resolved never invalidates it afterwards.
...
- Public blocks MUST NOT reference private blocks. This prevents public data from depending on content that non-recipient nodes cannot validate. A rotation block is readable by every node, so referencing one raises no such dependency and is permitted (see Validation rule 6).
