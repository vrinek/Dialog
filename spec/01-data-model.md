...
6. **A real date.** The date and time components MUST denote an existing instant: month `01`-`12`, a day that exists in that month of that year, hour `00`-`23`, minute `00`-`59`. Whether a day exists is decided in the **proleptic Gregorian calendar** — the calendar RFC 3339 uses, with its leap-year rules applied to every year, including the years before the Gregorian calendar was introduced in 1582. `1500-02-29T00:00:00Z` is therefore not a Dialog timestamp, even though February 1500 had 29 days in the Julian calendar then in civil use, while `1600-02-29T00:00:00Z` is one. The year `0000` is permitted, as the `timestamp` rule of the CDDL above allows, and is a leap year like any other year divisible by 400. A date taken from a Julian-calendar source is converted to the proleptic Gregorian calendar before the entity is created, exactly as a timestamp in another zone is converted to UTC.
