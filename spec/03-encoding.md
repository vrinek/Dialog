...
9. **Text map keys.** Every map key MUST be a text string (major type 3). Encoders MUST NOT emit, and decoders MUST reject, a map whose key is of any other major type, whether or not the definition of that map is known.
10. **Bounded nesting depth.** A *container* is an array or a map. A container's **nesting depth** is 1 when no container encloses it, and one more than the depth of the container immediately enclosing it otherwise; the outermost container of a document is therefore at depth 1. Items that are not containers — integers, byte strings, text strings, null — carry the depth of the container holding them and add nothing to it. A decimal fraction counts as exactly one container: tag 4 and the two-element array that is its content are a single level together, so a decimal fraction inside a container of depth 63 makes depth 64 and not 65. **No container in a Dialog document may have a nesting depth greater than 64.** Encoders MUST NOT produce a deeper structure, and decoders MUST reject one as malformed — as a rejection, never as a stack exhaustion.
...
*Informative.* Rule 9 is the schema-free half of rule 8. Every map this specification defines is keyed by text, so rule 8 already refuses an integer key inside a map whose definition is in hand; rule 9 says the same thing one layer lower, where a decoder holds bytes and no definition — the layer that meets the network first, and the one `vectors/dcbor.json` exercises. Without it two implementations could disagree about `a1 01 02` while satisfying every other rule, and a structure one of them hashes would be one the other refuses to parse.

*Informative.* Rule 10's bound is defensive, not expressive. The deepest structure this specification defines is about six levels — a block, its `ops` array, an operation, a molecule's `fillers` array, a filler, a scalar filler's value map — so 64 is an order of magnitude above anything a Dialog document needs, and no valid document is affected by it. What the number buys is that the accept/reject decision is the same everywhere: every recursive decoder needs some limit if a few kilobytes of `81 81 81 …` are not to exhaust its stack, and a limit each implementation picks for itself is a limit they disagree about. Since a document's identifier is the hash of its bytes, an implementation that stores what another cannot parse is exactly the interop failure the conformance vectors exist to prevent. `vectors/dcbor.json` pins both sides of the boundary.
