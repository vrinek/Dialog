...
**Revalidation on arrival.** A node that keeps a block as stored but unvalidated SHOULD validate that block again when a block it was waiting for **arrives**, whatever verdict the arriving block itself receives. The arrival is what settles the wait, for either cause a block can be waiting for:

- **A reference-resolution dependency** is settled by the arriving block being *readable*, because resolution reads blocks and not verdicts ("Resolution procedure" below, "Resolution reads blocks, not verdicts"). A block held as stored but unvalidated still defines the entities its operations create, so a digest waiting on it resolves the moment its bytes are there, and nothing about the arriving block's own chain standing can change which entity that digest names.
- **An ancestry dependency** is settled by the same event, even though rule 3 asks for a verdict rather than for bytes, because a node validates a block when it receives it: the arrival of the predecessor is the moment the predecessor's verdict exists. A waiter woken by a predecessor that was not itself accepted fails rule 3 again and goes on waiting, at the cost of one validation.

The third cause is settled by a key rather than by a block; a node that obtains the decryption key for a private block it holds SHOULD likewise validate the blocks that were waiting on that block's operations (see "Undecryptable reference handling" below).

The requirement binds a node that *keeps* held blocks. Discarding them instead remains conformant, as above — a node with no index of what waits on what is expected to discard and re-request rather than to hold blocks it never looks at again. What the SHOULD rules out is the third behaviour: keeping a block, receiving what it was waiting for, and leaving it undecided anyway. Two nodes doing that and not doing it reach different verdicts for the same blocks offered in the same order, which is what the validation rules exist to prevent.
...
**Resolution reads blocks, not verdicts.** A node MAY read an entity definition from any block it holds and can read that passes the block's **self-contained** checks — the checks a store performs on admission, none of which needs another block: that the bytes are the canonical dCBOR encoding of a block (rule 8), that the block carries exactly the fields its `type` declares and at least one operation (rules 1, 7 and 8), and that `sig` verifies against `pub` (rule 2). A node MUST NOT require, in addition, that the block be *valid*. A block whose chain standing nobody has established — one held as stored but unvalidated because its own `prev` has not arrived, one whose own references have not resolved, one that forks its author's chain, one that will turn out invalid when the rest of its chain arrives — still defines the entities its operations create, and a digest that resolves through it is resolved. Steps 4 and 5 above name blocks, not verdicts, and this is why.

*Rationale.* A definition is **self-certifying**, because Dialog addresses entities by content. An operation defines the digest D only if the entity's canonical bytes hash to D, and the reader establishes that for itself: it computes the digest from the bytes it read rather than taking any claim about them (see [01-data-model.md](01-data-model.md), "Content addressing"). Nothing about the source block's chain standing can change what those bytes hash to, so nothing about it changes which entity the digest names — the same definition would resolve identically from a different author's block, from a re-publication years later, or from the fat block of "Fat blocks" below. Reading only from valid blocks would mean something else entirely: resolving one digest would require the full validation of the foreign chain that carries it, back to its genesis block, and every block that chain in turn references. A node would fetch and validate a chain to read one sentence, which is exactly the cost the demand-driven model exists to avoid — and a block would then change verdict depending on the order its neighbours' blocks arrived in.

*What this does not change.* The permission is to read definitions during the resolution of rule 4, and nothing more:

- **L2 accumulation is untouched.** Only a valid block's operations enter the ontology graph ("Accumulation rules" above), and a stored but unvalidated block MUST NOT reach L2. A definition read from a block that is not valid supports the *referencing* block's verdict; it contributes nothing of its own to L2, and the entity enters the graph only when a valid block carrying it does.
- **Rules 6 and 10 still apply to the source block, as written.** A public block's `refs` MUST NOT name a private block whatever that block's own validity, and the own-chain prohibition is unaffected. Reading the operations of a private block still requires the decryption key; a node without one reaches outcome 3 above and not a definition.
- **Rule 3 is unaffected.** A predecessor MUST have been accepted as valid ([02-block-format.md](02-block-format.md), "Validation" rule 3). The asymmetry with rule 4 is deliberate: rule 3 asks whether an author's chain is intact, which is a fact about blocks and their order, while rule 4 asks what a digest names, which is a fact about content alone.
- **A verdict still moves in one direction.** A block declared valid on the strength of a definition read from an undecided block stays valid if that block later turns out invalid ([02-block-format.md](02-block-format.md), "Validation", "A verdict moves in one direction"). Nothing has to be undone: the entity the digest named is the entity it still names, and the referencing block's author published a correct reference to it either way.
