...
// TestValidateRFC3339 pins the reading of spec/01-data-model.md's "RFC 3339
// datetime string" this implementation applies; the open questions are
// todos/034.
func TestValidateRFC3339(t *testing.T) {
	valid := []string{
		...
		"2026-02-20T00:00:00.5Z",
		"2026-02-20T00:00:00.123456789Z",
	}
	for _, s := range valid {
		if err := ValidateRFC3339(s); err != nil {
			t.Errorf("ValidateRFC3339(%q) = %v, want nil", s, err)
		}
	}
	invalid := []string{
		"",
		"2026-02-20",
		"20260220T000000Z",
		"2026-02-20 00:00:00Z",
		"2026-02-20T00:00:00",  // no offset
	...
	for _, s := range invalid {
		if err := ValidateRFC3339(s); err == nil {
			t.Errorf("ValidateRFC3339(%q) = nil, want an error", s)
		}
	}
}
...
func TestDecodeFillerRejects(t *testing.T) {
	...
	cases := []struct {
		...
		{"range endpoint not RFC 3339", fillerMap(FillerScalar, dcbor.Map{
			{Key: keyFrom, Value: dcbor.Text("2026-02-20")},
			{Key: keyTo, Value: dcbor.Text("2026-02-20T23:59:59Z")},
		}), "is not an RFC 3339 datetime string"},
		{"range endpoint empty", fillerMap(FillerScalar, dcbor.Map{
			{Key: keyFrom, Value: dcbor.Text("2026-02-20T00:00:00Z")},
			{Key: keyTo, Value: dcbor.Text("")},
		}), "datetime is empty"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
