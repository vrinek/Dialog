// Development tools, versioned apart from the library.
//
// The main go.mod stays at exactly one dependency (golang.org/x/crypto); a
// linter is not something anybody importing this module should have to
// resolve. `go get -tool -modfile=tools.go.mod` records the tools here
// instead, and `go tool -modfile=tools.go.mod <name>` builds and runs them
// from these pinned versions.
module github.com/vrinek/Dialog/go/tools
